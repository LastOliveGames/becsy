...
abstract class WorkerBridge {
  ...
  private async receive(port: Port, laneId: number, message: Request | Response): Promise<void> {
    if (message.type === 'req') {
      try {
        ...
        port.postMessage({id: message.id, result, buffersPatch} as Response);
      } catch (e: any) {
        port.postMessage(
  ...
  protected async send(
    port: Port, action: string, args: any[], buffersPatch?: Patch
  ): Promise<any> {
    return new Promise((resolve, reject) => {
      const id = this.nextMessageId++;
      port.postMessage({type: 'req', id, action, args, buffersPatch});
...
export class Director extends WorkerBridge {
  ...
  async request(laneId: number, action: string, ...args: any[]): Promise<any> {
    return this.send(this.ports[laneId], action, args, this.dispatcher?.buffers.makePatch(laneId));
  }
...
class MainThreadLaborer extends Laborer implements DispatcherCore {
  ...
  execute(time?: number, delta?: number): Promise<void> {
    return this.request('execute', time, delta);
  }
